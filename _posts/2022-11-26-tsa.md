...
- Many useful time series can be specified using *difference equations* like $$y_t = k_1y_{t-1} + k_2y_{t-2} + \dots + k_ny_{t-n}$$
...
- Any time series combining exponential growth/decay and sinusoidal components can be modeled by a difference equation, or its matrix equivalent.
<!--more-->
...
A *regular* time series, where values are generated at evenly-spaced time intervals, is essentially a function from integers to real numbers. It can be defined as a function from $$\mathbb{N} \mapsto \mathbb{R}$$ if it starts at 0, or $$\mathbb{Z} \mapsto \mathbb{R}$$ if it starts at $$-\infty$$.

Just like $$y(x)=\sin (\frac{\pi}{k} x)$$ is a real-valued function, we can have $$y(t) = \sin (\frac \pi k t)$$ where $$t \in \mathbb{N}$$.

When we build our models, we try to limit the form of our functions to something general and complex enough to fit the processes we encounter, and also simple enough that we can parameterize if to fit our data.
...
This simple construct can generate any combination of exponential trend and periodic functions, which can model many real-world processes.
...
Understanding the process and how it responds to input is not by itself sufficient to map to unique values for any $$t$$; we also need some input. The difference equation or *recurrence relation* models the system dynamics, which determine a unique time series trajectory based on specified *initial conditions*. 
...
Given the roots $$\lambda_1$$ and $$\lambda_2$$, and initial conditions $$y_0$$ and $$y_1$$, we can solve a 2nd-order difference equation generally using determinants. We have a system of 2 equations with 2 unknowns $$a_1$$ and $$a_2$$.

$$y_0 = a_1 + a_2$$

$$y_1 = a_1 \lambda_1 + a_2 \lambda_2$$

$$D = \det \begin{bmatrix} 1 & 1 \\ \lambda_1 & \lambda_2 \end{bmatrix} = \lambda_2 - \lambda_1 $$

$$D_1 = \det \begin{bmatrix} y_0 & 1 \\ y_1 & \lambda_2 \end{bmatrix} = y_0 \lambda_2 - y_1 $$

$$D_2 = \det \begin{bmatrix} 1 & y_0 \\ \lambda_1 & y_1 \end{bmatrix} = y_1 - \lambda_1 y_0 $$

$$a_1 = \frac{D1}{D} = \frac{y_0 \lambda_2 - y_1}{\lambda_2 - \lambda_1} = \frac{2 \cdot 2 - 3}{2 - 1} = 1$$

$$a_2 = \frac{D2}{D} = \frac{y_1 - \lambda_1 y_0}{\lambda_2 - \lambda_1} = \frac{3 - 1 \cdot 2}{2 - 1} = 1$$
...
The method we just elaborated generalizes to higher-order linear difference equations:
...
With higher order difference equations we can get more complicated shapes with an exponential trend and more complicated interacting cycles.

Some tutorials on time series left me unsatisfied, because they dive straight into ARIMA without asking what a time series is and what types of processes ARIMA can model. 
...
If we have a data set, we can study it to see if it might come from this type of process and then parameterize it to fit our data and generate useful forecasts. Hopefully the next post will do a standard ARIMA model, and then we will move on to a deep learning model.
