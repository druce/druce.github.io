...
$$r$$ is termed the *modulus* and $$\theta$$ is termed the *argument*. It turns out that this is a good guess for the form of the solution:

$$y_t = \rho r^t \cos (\theta t + \omega)$$

for some $$\rho$$ and $$\omega$$, with $$\theta$$ and $$r$$ defined as above.
  - $$r$$: the growth/decay of the exponential envelope
  - $$\rho$$: the initial amplitude of the envelope when $$t=0$$
  - $$\theta$$: defines the period which is $$\frac{2\pi}{\theta}$$
  - $$\omega$$: the phase, how much the cosine wave is offset horizontally
...
This is the eigenvector equation in matrix form, it's saying if you apply the transformation $$F$$ to each of its eigenvectors $$\xi$$, you get the product of the eigenvalues $$D$$ and the eigenvectors $$\xi$$. Suppose we set our initial conditions to the eigenvectors of $$F$$. Then at each timestep $$y_t$$ is the previous $$y_{t-1}$$ multipled by the eigenvalues $$D$$. So we can say 

$$F^t \xi = D^t \xi$$

If $$y_0$$ is an eigenvector of $$F$$, then $$y_t$$ evolves as a function of the eigenvalues raised to powers of $$t$$. But guess what, we can express any initial conditions as a linear combination of the eigenvectors.
...
- A closed-form expression is $$y_t =  D^t \bf{a}$$, a linear combination of the $$\lambda^t$$s. We use our initial conditions to solve for the numeric value of $$\bf{a}$$. Each pair of complex roots is composed of complex conjugates, as are the corresponding $$a$$s, and all the $$i$$s cancel out to give a real cosine expression. 
