...
This is the eigenvector equation, it's saying if you apply the transformation $$F$$ to each of its eigenvectors $$\xi$$, you get the product of the eigenvalues and the eigenvectors. Suppose we set our initial conditions to the eigenvectors of $$F$$. Then at each timestep $$y_t$$ is the previous $$y_{t-1}$$ multipled by the eigenvalues. $$F^t \xi = D^t \xi$$, so we can see how $$y_t$$ evolves as a function of the eigenvalues raised to powers of $$t$$.
...
When we solve the system of equations, we solve for $$a$$s that capture the eigenvectors $$\xi$$ and $$\xi^{-1}$$ and the boundary counditions.
...
- Each pair of complex roots corresponds to an exponential growth / decay solution with an additional periodic component, $$y_t = \rho r^t \cos (\theta t + \omega)$$. Hence eigendecomposition is also known as spectral decomposition.

- The closed-form expression is $$y_t =  D^t \bf{a}$$, a linear combination of the $$\lambda^t$$s. We use our initial conditions to solve for the numeric values of $$\bf{a}$$.
