...
As an exercise, try to find the closed form of the Fibonacci sequence, where $$y_0=0$$, $$y_1=1$$, and $$y_t=y_{t-1} + y_{t-2}$$. First, find the roots $$\{\lambda_1, \lambda_2\}$$ of the characteristic equation $$-\lambda^2 + \lambda + 1 = 0$$. Then find $$a_1$$ and $$a_2$$ such that $$y_t = a_1 \lambda_1^t + a_2 \lambda_2^t$$ satisfies the initial conditions for $$t=0$$ and $$t=1$$. (solution in [TSA.ipynb](https://github.com/druce/TSA/blob/main/TSA.ipynb))
