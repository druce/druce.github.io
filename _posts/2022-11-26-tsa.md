...
When we have 2 roots $$\{\lambda_1, \lambda_2\}$$, then our final answer is actually some linear combination of the roots raised to the power of $$t$$:
...
- For any root $$\lambda$$, $$y_t = \lambda^t$$ satisfies the recurrence relationship, whether $$\lambda$$ is complex or otherwise.
- We can plunge forward and solve for $$a$$s such that $$y_t = a_1 \lambda_1^t + a_2 \lambda_2^t$$ satisfies the initial conditions. However the $$a$$s must be complex. and somehow the $$i$$s will cancel out for any $$t$$, since we know there are no $$i$$s in our time series, based on the initial conditions and recurrence relationship. 
...
$$r$$ is termed the *argument* and $$\theta$$ is termed the *modulus*. It turns out that the form of the solution is:

$$y_t = \rho r^t \cos (\theta t + \omega)$$

for some $$\rho$$ and $$\omega$$, with $$\theta$$ and $$r$$ defined as above. We'll try to work out why below, it gets a little gnarly. The reason is Euler's formula, which turns complex numbers raised to a power into trigonometric expressions: 
...
This is the eigenvector equation, it's saying if you apply the transformation $$F$$ to each of its eigenvectors $$\xi$$, you get the product of the eigenvalues $$D$$ and the eigenvectors $$\xi$$. Suppose we set our initial conditions to the eigenvectors of $$F$$. Then at each timestep $$y_t$$ is the previous $$y_{t-1}$$ multipled by the eigenvalues $$D$$. So we can say 

$$F^t \xi = D^t \xi$$

$$y_t$$ evolves as a function of the eigenvalues raised to powers of $$t$$.
...
- Finally multiplying by $$\xi$$ restores our initial coordinate system. 
...
- Each real root corresponds to an exponential growth / decay solution with no periodicity, $$y_t = \lambda^t$$.

- Each pair of complex roots corresponds to an exponential growth / decay solution with an additional periodic component, $$y_t = \rho r^t \cos (\theta t + \omega)$$. For this reason, eigendecomposition is also known as spectral decomposition. 

- A closed-form expression is $$y_t =  D^t \bf{a}$$, a linear combination of the $$\lambda^t$$s. We use our initial conditions to solve for the numeric values of $$\bf{a}$$. Each pair of complex roots is composed of complex conjugates, as are the corresponding $$a$$s, and all the $$i$$s cancel out to give a real cosine expression. 

- Asymptotically, the root (eigenvalue) with largest absolute value (modulus) will dominate. If it is larger than 1 in absolute value (modulus) our time series diverges. If it is less than 1 in absolute value (modulus) our time series goes to 0. If it is equal to 1 in absolute value (modulus) our time series is asymptotically stable in the region given by the initial conditions.
...
- Steve Brunton a/k/a Eigensteve has some [excellent videos on dynamic systems](https://www.youtube.com/watch?v=9fQkLQZe3u8&list=PLMrJAkhIeNNTYaOnVI3QpH7jgULnAmvPA) in general (not time series specifically but a linear difference equation becomes a linear ordinary differential equation as the size of the timesteps goes to 0).
