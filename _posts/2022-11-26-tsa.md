...
- Somehow $$a_1 \lambda_1^t + a_2 \lambda_2^t$$ generate a cosine function of some amplitude and period and phase. 

The $$a_1 \lambda_1^t + a_2 \lambda_2^t$$ with complex $$a$$s and $$\lambda$$s is not *wrong*. But it would be *nice* to just find the solution with a cosine function and real amplitude and phase.
...
It turns out that the form of the solution is:
...
(Note that $$\bf{v_0}$$ is in reverse order from the way we apply the $$F$$. $$V$$ is known as the [Vandermonde matrix](https://en.wikipedia.org/wiki/Vandermonde_matrix).)
...
- A closed-form expression is $$y_t =  D^t \bf{a}$$, a linear combination of the $$\lambda^t$$s. We use our initial conditions to solve for the numeric values of $$\bf{a}$$.
