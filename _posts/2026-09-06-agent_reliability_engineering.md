...
1. **Observability: tracing and run manifests.** Every run logs a comprehensive persistent trace with a manifest of exactly what it ran on: the starting state (input files, hashes, as-of dates), prompt versions (ideally tied to an immutable prompt registry like Langfuse), model string, tool and skill calls with versions, token counts, latency, and exit status per step. This lets you do error analysis, reproduce a gold output exactly, and satisfy an auditor. The logged manifest and trace ship with the gold output as first-class deliverables. A report you can't reconstruct is a report you can't defend.
...
2. **[Context management](https://agentic-ai.readthedocs.io/en/latest/ContextEngineering/anthropic/): Keep It Simple, Stupid.** A series of small prompts is more predictable, verifiable, and steerable than one mega prompt. Small prompts reduce and control what's in context, avoiding [context rot](https://www.trychroma.com/research/context-rot), and each prompt is easier to reason about, test, and evaluate in isolation. Stay under 50% of the context window during runs; degradation sets in well before the advertised window fills. High context usage is a signal to decompose.
   ...
   - **An external orchestrator** running each  prompts in a fully independent session — e.g., a Python script where each step processes artifacts from the previous step and writes new ones, invoking `claude -p` per step.
...
6. **Karpathy wiki methodology**. A simple approach to agent memory is to use a wiki with a specified structure as the silver store, with links back to the bronze canonical sources. Andrej Karpathy published an [LLM Wiki repo](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f) with a methodology for auto-maintaining a structured wiki. Read sources and extract relevant facts into the silver wiki with a specified structure, then build the final product from the wiki.

   There are several agent memory modules and SaaS services. They can be convenient but don't relieve you of being thoughtful about how you structure the data for your workflows. A wiki is a simple place to start and will work well when the whole wiki fits easily within the LLM context. As the amount of data grows, you will need to think harder about structuring agent memory to always give the right context to the LLM, [which may involve different types of memory and database engines](https://www.memoryplugin.com/wiki/index.html), and simple tools and skills to let the LLM extract exactly what it needs when it needs it.
...
8. **Hard gates, enforced by contracts and deterministic code.** Sanity-check before each step (are prerequisites in place?) and after (does the output look clean?). Before a writing step, for instance: is silver data present, recent, and clean? Render and inspect final artifacts — open the PDF, count the pages, check the charts rendered. Enforce hard budgets (output length, tokens, wall clock) and cap retries, e.g., at three attempts, so no gate becomes an infinite loop. In more complex topologies, add loop detection for orchestrator/subagent ping-pong, where agents bounce work back and forth without making progress.
...
10. **[Evals](https://hamel.dev/blog/posts/evals-faq/): unit tests for every step.** Every hard and soft gate maps naturally to an eval you can run as a suite in e.g. [promptfoo](https://www.promptfoo.dev/) or [Langfuse](https://langfuse.com/). An eval suite lets you pick the cheapest model that consistently gets the job done. When the model version or anything else changes, you rerun the suite and see what regressed.

    - **Measure against a contract using a scoring rubric, to a structured schema.** See above on soft gates.
    ...
    - **Generate evals from observed and edge cases.** Follow [Hamel Husain and Shreya Shankar's error-analysis loop](https://www.oreilly.com/library/view/evals-for-ai/9798341660717/): sample production traces, label the failures, cluster them, write an eval per cluster using the hardest cases. Your eval suite should be a fossil record of everything that has actually gone wrong.
...
1. **Parallelize everything**. Use async/await or tell skills to run these tasks in parallel whenever dependencies allow.
...
5. **Treat prompts as code**. For higher maturity deployments, prompts should be in CI and/or a prompt repo like Langfuse, which makes it easy for non-devs to iterate prompts. When a model updates you can eval current and previous prompt versions to catch new problems and regressions.
...
7. **The 4 important sources of metrics about how well the agent is working**:
   1. Track runtime evals and gate trips over time.
   2. Usage level/growth. If people use it, it's probably useful.
   ...
   4. Controlled experiments / benchmarks: Run the agent end-to-end on real-world out-of-sample tasks and do human/automated scoring of how well it works. To this end, a good harness for A/B testing, letting humans evaluate and label alternative outputs/traces, can be worth its weight in gold.
