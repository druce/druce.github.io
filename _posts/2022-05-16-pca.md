...
I was refreshing my understanding of Principal Component Analysis (PCA) and the Singular Value Decomposition (SVD) and came across this [excellent method](http://www.pyrunner.com/weblog/2016/08/01/optimal-svht/) for selecting an optimal rank for dimensionality reduction.
...
- An orthogonal matrix is just a rotation (and optional reflection). PCA is simply a change of basis to a better basis for this data set. What is a good basis? Suppose you are analyzing a spiral galaxy. Some weird basis from a random point in random directions may not be a good basis. A good set of coordinates might be:
   ...
   - From the point of view of this basis, covariances are eliminated, computation is easier, and interpretation can be more intuitive. And we can reduce dimensionality by ignoring dimensions that are mostly noise.
