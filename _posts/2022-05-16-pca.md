...
- The principal components are defined as $$P = U\Sigma$$, so by substitution we get the original data back as $$A=PV^T$$. Multiplying each side by $$V$$ we get $$P=AV$$. (The inverse of an orthogonal matrix is its transpose.) So matrix $$V$$ is the loadings that transform original feature columns into principal components; and vice versa, $$V^T$$ is the loadings that transform principal component columns inot original feature columns. Each principal component is a weighted average of the feature columns, where the weights form an orthogonal matrix.[^2] [^3]
- An orthogonal matrix is just a rotation (and optional reflection). PCA is simply a change of basis to a better basis for this data set. What is a good basis? Suppose you are analyzing a spiral galaxy. Some weird basis from a random point in random directions may not be a good basis. A good set of coordinates might be:
   ...
   - From the point of view of this basis, covariances are eliminated, computation is easier, and interpretation can be more intuitive. And we can possibly reduce dimensionality by ignoring dimensions that are mostly noise.
