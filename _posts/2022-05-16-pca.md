...
- The principal components are defined as $$P = U\Sigma$$, so by substitution we get the original data back as $$A=PV^T$$. Multiplying each side by $$V$$ we get $$P=AV$$. (The inverse of an orthogonal matrix is its transpose.) So matrix $$V$$ is the loadings that transform original feature columns into principal components; and vice versa, $$V^T$$ is the loadings that transform principal component columns inot original feature columns. Each principal component is a weighted average of the feature columns, where the weights form an orthogonal matrix.[^1] [^2]
- An orthogonal matrix is just a rotation (and optional reflection). PCA is simply a change of basis to a better basis for this data set. What is a good basis? Suppose you are analyzing a spiral galaxy. Some weird basis from a random point in random directions may not be a good basis. A good set of coordinates might be:
   ...
   - This basis eliminates covariances, makes computation easier, and interpretation more intuitive.
...
Usually, we don't know the $$\sigma$$ of the noise. Gavish-Donoho gives us a way to estimate the threshold based on the median singular value and the aspect ratio of the matrix in equation 11 of the paper. This equation predicts at what point we are adding more error than signal to the estimated low-rank matrix. When we apply equation 11 we get almost the identical threshold (51.8 vs. 51.6) and the identical rank-2 approximation.
...
The rank-29 compression is acceptable and is close to what we get by eyeballing the elbow in the singular values. But it doesn't seem to have performed any noise reduction. Adding more dimensions does improve the image. It's pretty clear that by adding more ranks, we get more photographic detail, which isn't true white noise. 

One possible problem with the Gavish-Donoho threshold is that it's trying to minimize the MSE between a target matrix and the truncated representation, assuming the target is a low rank matrix plus noise. But true optimal truncation depends on what we are going to use low-rank output for, and what our loss function is. Sometimes the low-rank vectors aren't white noise and the information we need to process downstream is in those low-rank vectors. If we're trying to read an inscription on a ring in this photo, we might be better off with full-rank data.

Nevertheless, the Gavish-Donoho optimal truncation threshold is a good starting point and adding additional ranks for a margin of safety seems to capture the essential information of this photo for downstream processing like facial recognition or image classification. Even tripling the size of the matrix adds only a modest improvement. 
