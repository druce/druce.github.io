...
The rank-29 compression is acceptable and is close to what we get by eyeballing the elbow in the singular values. But it doesn't seem to have performed any noise reduction. Adding more dimensions does improve the image, we get more photographic detail, which isn't true white noise. 

One possible problem with the Gavish-Donoho threshold is that it's trying to minimize the MSE between a target matrix and the truncated representation, assuming the target is a low rank matrix plus noise. But true optimal truncation depends on what we are going to use low-rank output for, and what our loss function is. Sometimes the low-rank vectors aren't white noise and the information we need to process downstream is in those low-rank vectors. If we're trying to read an inscription on a ring in this photo, we might be better off with full-rank data. The Goethe quote up top seems *à propos*.
