...
If we plot our principal components, we have our unrotated data back. The PCA rotation unscrambles or unf\*\*ks the basis, whether it's in 2 or higher dimensions.
...
In Figure 11, it's easy to see in this contrived example where the rank-2 singular values of our noisy image run out of significance and the noise starts to dominate.

In this example, we know the $$\sigma$$ of our noise and number of rows $$m$$, so according to the Gavish-Donoho paper, the truncation threshold for a square matrix can be computed as $$\frac{4}{\sqrt{3}}\sqrt m \sigma$$, approximately 51.6. We select all the singular values larger than the threshold, so we can truncate the rank to 2. In other words we take our original SVD
$$A=U \Sigma V^T$$
...
We are still transforming our 500x500 matrix $$\in\mathbb{R}^{500\times 500}$$ into another 500x500 matrix, but projecting onto a lower-dimensional rank-2 plane subspace of $$\mathbb{R}^{500\times 500}$$.
...
Usually, we don't know the $$\sigma$$ of the noise. Gavish-Donoho gives us a way to estimate the threshold based on the median singular value and the aspect ration of the matrix ($$\beta$$) in equation 11 of the paper. This models at what point we are adding more error than signal to the estimated low-rank matrix. When we apply equation 11 we get almost the identical threshold (51.8 vs. 51.6) and the same rank-2 approximation.
...
The rank-29 compression is acceptable and is close to what we get by eyeballing the elbow in the singular values. But it didn't seem to perform any noise reduction. And adding more dimensions does improve the image. It's pretty clear that by adding more ranks, we get more photographic detail, which isn't true white noise. 

One possible problem with the Gavish-Donoho threshold is that it's trying to minimize the MSE between a target matrix and the truncated representation, assuming the target is a known low rank plus noise. But true optimal truncation depends on what we are going to use low-rank output for, and what our loss function is. Sometimes the low-rank vectors aren't white noise and the information we need to predict is in there. If we're trying to read an inscription on a ring in this photo, we might be better off with full-rank data.

Nevertheless, using the Gavish-Donoho optimal truncation threshold as a starting point and adding additional ranks for a margin of safety seems to capture the essential information for downstream processing like facial recognition or image classification. Even tripling the size of the matrix adds only a modest improvement. 
