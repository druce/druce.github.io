...
- Geometrically, every linear algebra transformation is rotation, dilation, reflection or projection, and compositions thereof. 
...
- The labor market principal component might have big loadings on employment growth, wages, big negative loadings on the unemployment rate and jobless claims, and then it must subtract enough of the other features to eliminate any correlation with any other principal component. The labor principal component's singular value would measure its contribution to the variance.
...
[^2]: The same applies to rows.  Suppose we have a data set of observations of organisms, with feature columns like height, weight, body temperature, life expectancy, etc. When we do PCA, suppose the most significant latent variable represents 'animal-ness'. Another might be 'plant-ness', and another 'micro-organism-ness.' PC1 is a weighted average of original feature columns. $$V^T$$ contains, for each row, the weights we have to apply on each PC to get the original data row back. $$V^T$$ column 1 ($$V$$ row 1) represents the 'animal-ness' of each feature column Similarly, row 1 of $$U$$ represents the animal-ness of each observation row. Mathematically, if $$A=U\Sigma V^T$$, then $$A^T=V\Sigma U^T$$, which is the SVD of the transpose (because $$(AB)^T = B^TA^T$$ and the transpose of a diagonal matrix like $$\Sigma$$ is itself). You could maybe argue that $$V$$ column 1 represents pure 'animal-ness' as a combination of features, and that $$U$$ column 1 represents pure 'animal-ness' as a combination of individuals.
...
If we multiply our left singular values by the singular values, we got our principal components. In this example we have our original unrotated data back. The PCA rotation unscrambles or unf\*\*ks the basis, whether it's in 2 or higher dimensions.
...
In Figure 11, it's easy to see in this contrived example where the rank-2 singular values of our noisy image run out of significance and the noise starts to dominate.
  ...
  * [ ] In this example, we know the $$\sigma$$ of our noise and number of rows $$m$$. According to the Gavish-Donoho paper, the truncation threshold for a square matrix can be computed as $$\frac{4}{\sqrt{3}}\sqrt m \sigma$$, here approximately 51.6. We select all the singular values larger than the threshold. This correctly truncates the rank to 2. In other words we take our original SVD
$$A=U \Sigma V^T$$
and we keep only the first 2 columns of $$U$$, the top left 2x2 part of $$\Sigma$$, and the first 2 rows of $$V^T$$. We recompute $$A$$ from the truncated SVD matrices of the noisy image as
...
Usually, we don't know the $$\sigma$$ of the noise. Gavish-Donoho gives us a way to estimate the threshold based on the median singular value and the aspect ratio of the matrix in equation 11 of the paper. This models at what point we are adding more error than signal to the estimated low-rank matrix. When we apply equation 11 we get almost the identical threshold (51.8 vs. 51.6) and the identical rank-2 approximation.
...
One possible problem with the Gavish-Donoho threshold is that it's trying to minimize the MSE between a target matrix and the truncated representation, assuming the target is a low rank matrix plus noise. But true optimal truncation depends on what we are going to use low-rank output for, and what our loss function is. Sometimes the low-rank vectors aren't white noise and the information we need to predict is in there. If we're trying to read an inscription on a ring in this photo, we might be better off with full-rank data.
