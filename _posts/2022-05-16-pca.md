...
---
>PCA is an essential data science tool which uses the SVD to break down the linear relationships in data. The Gavish-Donoho optimal truncation threshold is a systematic way to select a good threshold for dimensionality reduction.
...
- The principal components are defined as $$P = U\Sigma$$, so by substitution we get the original data back as $$A=PV^T$$. Multiplying each side by $$V$$ we get $$P=AV$$. (The inverse of an orthogonal matrix is its transpose.) So matrix $$V$$ is the loadings that transform original feature columns into principal components; and vice versa, $$V^T$$ is the loadings that transform principal component columns back into original feature columns. Each principal component is a weighted average of the feature columns, where the weights form an orthogonal matrix.[^2] 
