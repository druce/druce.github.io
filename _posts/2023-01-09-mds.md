...
- Extract, Load, and Transform (ELT) tools, to move data into the data warehouse and :
	...
	-  Scheduling / orchestration tools to detect new data and trigger jobs when needed: maybe on a schedule, maybe monitoring a local directory, maybe polling a remote directory, maybe receiving emails or other signals.
...
- Generally too many UX suprises.
...
- Containerize the data stack-in-a-box. Make a [Dockerfile](https://github.com/druce/MTA/blob/main/Dockerfile) with installs of DuckDB, dbt, and Superset and other tools. By the time I got everthing in there that I wanted, it became a 20GB image and that's kind of an abuse of Docker. 
