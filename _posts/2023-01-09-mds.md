...
This works pretty well ([code here](https://github.com/druce/MTA)). It seems to be an order of magnitude faster than raw pandas on 'pretty big' data. It's prettyy cool, if you grew up in a world where Google didn't exist, to compute Google-style on your laptop or a $20 Hetzner or AWS box. It's similar enough to production-ready processes that when the time comes to move from your laptop to 'real' enterprise infrastructure, whether SaaS or 'on-prem', you are using mostly the same approaches. 

DuckDB has limitations. It is single-user. It writes to a single file. But it leverages all your RAM and threads well. If you sub Spark for DuckDB, this data-warehouse-in-a-box starts is an OK starting point for an enterprise stack.
...
At this point, rolling your own on-prem modern data stack from open source is for teams with scale. Or penniless startups and hobbyists who don't want to pay for SaaS services. Which is probably not most teams. Regardless, in the rest of this post, we stand up our own small environment for a data app.
...
After a fair amount of wrangling, I get a dashboard that looks like this (2022 filter applied):

![Superset dashboard](/assets/2023/mta-dashboard.jpg)

Superset is a poor man's Tableau. It's powerful but complex and quirky.[^4] It was not as straightforward to create this as I expected. That being said, we can filter by a date range and see a comparison with pre-pandemic and pandemic periods, we can filter by Manhattan below 63 St, by day of the week, or anything reasonable, and see a comparison with those days during pandemic and pre-pandemic. We can make a map widget and sort any table column to see top/bottom stations. 

I feel like this dashboard could use a little more work, formatting, color-code maps by entries/growth etc. But I've spent enough time and I've seen enough. For real-time dashboards I might go with [Grafana](https://grafana.com/) which is known for real-time monitoring. (Superset is also designed for real-time updates via [Apache Druid](https://druid.apache.org/)). Since I love [Plotly](https://plotly.com/) and can code up nice charts in Python, I might go with Plotly [Dash](https://plotly.com/dash/) for dashboards, where I feel way more confidence I can configure every label and tickmark's style and size without going down a rabbit hole. For point-and-click no-code BI I might go with Tableau or PowerBI. Superset is not going to power state-of-the-art NY Times dataviz. But Superset is pretty good for a lot of purposes and has a lot of potential.
...
[^4]: As an example of a quirk, when I try to export the code for my dashboard, I get "TypeError: send_file() got an unexpected keyword argument 'cache_timeout'". And much of the point of writing this was to share starter code. Not the first time it did something unexpected. Promising but it's not Tableau or PowerBI and YMMV.
