...
### Hardware
 ...
 - I am running on a 2019 MacBook Pro with 32GB and 8 cores. If you run on a cloud instance, I would recommend something like [r6a.xlarge](https://aws.amazon.com/ec2/pricing/on-demand/): AMD CPU, 32GB RAM, 4 cores. 
...
### Software
 ...
 - [Clone the repo](https://github.com/druce/MTA)
~~~ shell
...
- Download and install the appropriate [Anaconda](https://www.anaconda.com/products/distribution).
...
# 4. dbt Initial DB Build / Transform

### Project overview

- For an idea of what we will be doing, check out [MTA-DuckDB.ipynb](https://localhost:8888/notebooks/MTA/MTA-DuckDB.ipynb)

### Run dbt

- Edit dbt's profiles.yml config file to point to the directory where you cloned the repo. Put it in your home directory under `~/.dbt/`. dbt  keeps these credentials and connection profiles here by default to avoid risk of putting credentials in git repos.
...
- On my Mac, this takes about 4 minutes to run including the initial DB load from .txt files.
- For comparison, the first version that of the analysis that I wrote in pure Pandas without DuckDB takes about 30 minutes. With modin multi-threaded dataframes as a drop-in replacement for Pandas this is cut to about 20 minutes. The comparison is not a strictly valid benchmark since the transforms are not exactly the same, I cleaned things up as I want along. But it should give an idea of the benefits of the modern data stack.
...
- part 5, dbt and build  walkthrough

- show output and build times
- show doc with the diagram
