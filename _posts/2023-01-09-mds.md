...
But times have changed. You can fit *a lot* on a box. I have a 2019 Intel MacBook Pro with 32GB of RAM. So with the right stack, I can effectively analyze maybe 20 years of MTA data down to individual turnstiles. Which is 'pretty big' data.
...
DuckDB has limitations. It is single-user. It writes to a single file. If your data does not fit in RAM on your single machine, it will just fail. But if you sub Spark for DuckDB, this data-warehouse-in-a-box starts is a decent starting point for an enterprise stack.
