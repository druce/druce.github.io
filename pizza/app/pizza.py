...
_api_key = None
_gmaps_configured = False
_yelp_api = None
_foursquare_creds = None
...
def _ensure_gmaps_configured():
    global _gmaps_configured
    if not _gmaps_configured:
        api_key = _load_api_key()
        gmaps.configure(api_key=api_key)
        _gmaps_configured = True


def _get_yelp_api():
    global _yelp_api
    if _yelp_api is None:
        # https://github.com/gfairchild/yelpapi
        with open('secrets/yelpkey.txt') as f:
            yelp_key = f.readline().strip()
        _yelp_api = YelpAPI(yelp_key)
    return _yelp_api


def _get_foursquare_creds():
    global _foursquare_creds
    if _foursquare_creds is None:
        ...
        _foursquare_creds = (foursquare_id, foursquare_secret)
    return _foursquare_creds
...
def foursquare_get_df(location_coords, keyword):
    location_str = "%.7f,%.7f" % location_coords
    foursquare_id, foursquare_secret = _get_foursquare_creds()
