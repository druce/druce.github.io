...
# Security: Whitelist allowed keywords and locations
ALLOWED_KEYWORDS = {'pizza', 'coffee', 'icecream'}
ALLOWED_LOCATIONS = {'midtown', 'downtown', 'uppereastside', 'upperwestside',
                     'brooklynheights', 'grandarmyplaza', 'bayridge', 'williamsburg'}
...
if not os.path.exists('logs'):
    os.mkdir('logs')
file_handler = RotatingFileHandler('logs/pizza-app.log', maxBytes=10240000, backupCount=10)
...
app.logger.info('Pizza app startup')
    ...
    if args:
        ...
        if location and keyword:
            ...
            if keyword not in ALLOWED_KEYWORDS:
                app.logger.warning(f'Invalid keyword attempt: {keyword} from {request.remote_addr}')
                return Response("Invalid keyword", status=400)
            if location not in ALLOWED_LOCATIONS:
                app.logger.warning(f'Invalid location attempt: {location} from {request.remote_addr}')
        ...
        # if lat and lng provided and enabled, search online using service APIs
            ...
            try:
                ...
                app.logger.warning(f'Invalid coordinates: lat={lat}, lng={lng} from {request.remote_addr}')
            ...
            app.logger.warning(f'Invalid query string from {request.remote_addr}: {request.args}')
            return Response("Invalid query string received", status=400)
    else:
        app.logger.warning(f'No query string from {request.remote_addr}')
        return Response("No query string received", status=400)
